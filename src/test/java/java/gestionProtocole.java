	...
	public String travaille (String requete) throws SQLException, ClassNotFoundException {
                ...
                switch (param[0]) {
                    ...
                    case "CONFIDENTIALITE":
                        ...
                        //Mettre client a l'etat connecte
                        ...
                        // Afficher les messages d'un etudiant
                        
                        case "LIREMESSAGE" :
                            int numEtudiant = Integer.parseInt(param[1]);
                            try
                            {
                                String res= mes1.LireMessage(numEtudiant);
                                resultat="OK_LECTURE_MESSAGE  "+res;
                                mes1.effacerMessage(numEtudiant);
                            }
                            catch (Exception e)
                            {
                                resultat="NON_OK_LECTURE_MESSAGE";
                            }
                            break;
                            
                        case "ENVOYERMESSAGEDIFF":
                            try
                            {
                               int numEtudiantExpediteur= Integer.parseInt(param[1]);
                               int numEtudiantDest = Integer.parseInt(param[2]);
                               String message = param[3];
                               boolean res=mes1.envoyerMessage(numEtudiantExpediteur, numEtudiantDest, message);
                               if (res=true)
                               {
                                   
                                   resultat= "OK_ENVOYER_MESSAGE";
                               }
                               else
                               {
                                   resultat="NOK_ENVOYER_MESSAGE";
                               }
                            }
                            catch (Exception e)
                            {
                        ...
                        //Affiche un profil complet en fonction des des parametres de confidentialité
                        case "AFFICHER_PROFIL_COMPLET":
                            try
                            {
                                numEtudiant= Integer.parseInt(param[1]);
                                String res=an1.afficherProfilEtudiantComplet(numEtudiant); 
                                resultat= "OK_AFFICHER_PROFIL_COMPLET "+res;
                            }
                            catch (Exception e)
                            {
                                resultat="NON_OK_AFFICHER_PROFIL_COMPLET";
                            }
                        break;
                        
                        
                        //Recommande une competence a un utilisateur
                        case "RECOMMANDERCOMPETENCE":
                            try
                            {
                                int id_competence= Integer.parseInt(param[1]);
                                int numEtudiantRecommander= Integer.parseInt(param[2]);
                                int numEtudiantRecommandeur= Integer.parseInt(param[3]);
                                int res=an1.recommanderCompetence(id_competence,numEtudiantRecommander,numEtudiantRecommandeur);
                                if (res==1)
                                {
                                    resultat="OK_RECOMMANDER_COMPETENCE ";
                                }
                                if (res==2)
                                {
                                    String resu="Vous avez deja recommander cette compéténces";
                                    resultat="OK_RECOMMANDER_COMPETENCE "+resu;
                                }
                                if (res==-1)
                                {
                                    resultat="NOK_RECOMMANDER_COMPETENCE "+res;
                                }
                                
                            }
                            catch (Exception e)
                            {
                                resultat="ERREUR_OK_AJOUT_COMPETENCE";
                            }
                            break;
                            
                        case "SUPPRIMER_RECOMMANDATION_COMPETENCE":
                            try
                            {
                                int id_competence= Integer.parseInt(param[1]);
                                int numEtudiantRecommander= Integer.parseInt(param[2]);
                                int numEtudiantRecommandeur= Integer.parseInt(param[3]);
                                int res=an1.SupprimerRecommandationCompetence(id_competence,numEtudiantRecommander,numEtudiantRecommandeur);
                                if (res==1)
                                {
                                    resultat="OK_SUPPRIMER_RECOMMANDATION_COMPETENCE "+res;
                                }
                                if (res==2)
                                {
                                    String resu="Vous n'avez pas recommander cette compétence";
                                    resultat="OK_SUPPRIMER_RECOMMANDATION_COMPETENCE "+resu;
                                }
                                if (res==-1)
                                {
                                    resultat="NOK_SUPPRIMER_RECOMMANDATION_COMPETENCE "+res;
                                }
                                
                            }
                            catch (Exception e)
                            {
                                resultat="ERUUER_SUPPRIMER_RECOMMANDATION_COMPETENCE";
                            }
                            break;
                               
                           
                        case "AFFICHER_LISTE_COMPETENCE" :
                                            try
                                            {
                                                String res = an1.afficherListeCompetences();
                                                resultat= "OK_AFFICHER_LISTE_COMPETENCE  "+res;

                                            }
                                            catch (Exception e) 
                                            {
                                                resultat = "NON_AFFICHER_LISTE_COMPETENCE";
                                            }
                                            break;                         
     //Permet de liker un profil " mettre un +1"
                        case "LIKER_PROFIL":
                            try
                            {
                               int num_etudiantLikeur= Integer.parseInt(param[1]);
                               int num_etudiantLiker= Integer.parseInt(param[2]);
                               int res=an1.liker_profil(num_etudiantLikeur,num_etudiantLiker);
                               if (res==1)
                               {
                                  resultat= "OK_LIKER_PROFIL";
                               }
                               if (res==2)
                               {
                                  String resu="Profil déjà liké";
                                  resultat= "OK_LIKER_PROFIL "+resu;
                               }
                               if (res==-1)
                               {
                                  resultat= "OK_LIKER_PROFIL "+res;
                               }
